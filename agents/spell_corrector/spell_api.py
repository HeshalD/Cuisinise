...
def _lazy_imports():
    ...
    if mlm_fill_mask is None:
        try:
            ...
            device_arg = torch_device_index if torch_device_str == "cuda" else -1
            globals()["mlm_fill_mask"] = _pipeline("fill-mask", model="bert-base-uncased", device=device_arg)
        except Exception:
    ...
    if sentence_model is None:
        try:
            ...
            globals()["sentence_model"] = _ST("all-MiniLM-L6-v2", device=torch_device_str)
        except Exception:
    ...
    if not domain_vocab:
        domain_vocab = set([
            ...
            # locations of interest (extend as needed)
